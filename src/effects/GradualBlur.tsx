...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      const k = cap(key);
      const breakpoints = config as Record<string, string | undefined>;
      if (w <= 480 && breakpoints['mobile' + k]) v = breakpoints['mobile' + k];
      else if (w <= 768 && breakpoints['tablet' + k]) v = breakpoints['tablet' + k];
      else if (w <= 1024 && breakpoints['desktop' + k]) v = breakpoints['desktop' + k];
      setVal(v);
    };
